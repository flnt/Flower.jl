...
function find_position_hill_x(grid, MIXED, h)
    @unpack x, y, mid_point = grid
    H = x[1,:]
    radius = Vector{Float64}(undef,0)
    for II in MIXED
        try
            radius_ = H[II.I[2]]+h*mid_point[II].x
...
function find_position_hill_x_max(grid, MIXED, h)
    @unpack x, y, mid_point = grid
    H = x[1,:]
    radius = Vector{Float64}(undef,0)
    for II in MIXED
    ...
    return mean(radius), findmax(radius)[1]
end

function run_forward2(num, grid, grid_u, grid_v,
    opS, opL, phS, phL, fwd;
    periodic_x = false,
    periodic_y = false,
    BC_TS = Boundaries(
    ...
    while current_i < max_iterations + 1
        ...
        av_height1, max_height = find_position_hill_x_max(grid, MIXED, Δ) 

        av_height1 += 0.5 - 0.05
        max_height += 0.5 - 0.05

        #@show (av_height1, max_height)
        if max_height > 0.9 || norm(phL.u) > 1e30 || norm(phL.v) > 1e30
            @show ("exit")
...
Ra_array = [1e6] #[1e3, 1e4, 1e5, 1e6]
LH_array = [5.0] #[2.5, 5.0, 10.0]
TM_array = [0.] #[0., 0.1, 0.2]
...
for vθd in TM_array
    for vλ in LH_array
        for vRa in Ra_array
            ...
            @time MIXED, SOLID, LIQUID = run_forward2(num, gp, gu, gv,
                opS, opL, phS, phL, fwd,
                periodic_y = true,
                BC_TL = Boundaries(
...
#            JLD2.@save "/home/tfullana/rayleigh-data/St_$(St)-nx_$(nx)-ny_$(ny)-ratio_$(ratio)-maxiter_$(@sprintf("%.1e", num.max_iterations))-TM_$(num.θd)-T1_$(T1)-T2_$(T2)-λ_$(λ)-Ra_$(@sprintf("%.1e", Ra)).jld2" St num gp gu gv phS phL fwd
            JLD2.@save "/media/tf/be46b01f-eb0c-4298-a160-d10e4e87b3b9/RB_opt_data/opt_St_$(St)-nx_$(nx)-ny_$(ny)-ratio_$(ratio)-maxiter_$(@sprintf("%.1e", num.max_iterations))-TM_$(num.θd)-T1_$(T1)-T2_$(T2)-λ_$(λ)-Ra_$(@sprintf("%.1e", Ra)).jld2" St num gp gu gv phS phL fwd
        end
    end
end
