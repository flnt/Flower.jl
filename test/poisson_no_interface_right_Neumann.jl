...
for (i,n) in enumerate(npts)
    ...
    right = Neumann(val = sign_Poisson * ∇fx.(x_bc_right, gp.y[:,gp.ny])  * sign_right),
    ...
    @testset "BC" begin
        ...
        @testset "Left border rhs" begin
            @test vecb_L(rhs,gp)./gp.dy[:,1] ≈  0 * sign_left .* ones(gp.ny) atol = test_tolerance 
            #-10: -BC because left face (divergence theorem)
        end

        @testset "Right border rhs" begin
            @test vecb_R(rhs,gp)./gp.dx[:,end] ≈  sign_Poisson * ∇fx.(x_bc_right, gp.y[:,gp.nx]) * sign_right .* ones(gp.ny) atol = test_tolerance
        end
    ...
    @testset "Corner border" begin
        ...
        print("\n L/n ", liquid_height," i_corner_vecb_left ",i_corner_vecb_left," \n")
        ...
        @testset "A[i_corner_vecb_left,i_corner_vecb_left]" begin
            @test A[i_corner_vecb_left,i_corner_vecb_left] ≈ liquid_height atol = test_tolerance
        end
    ...
    end #at from the wall


    IIwall = CartesianIndex(j,gp.nx)
    tmp = lexicographic(IIwall,gp.ny)

    # i_corner_vecb_left = (num.nLS + 1) * ni + j

    i_corner_vecb_right = (num.nLS + 1) * ni + gp.ny + gp.nx + j 
    ...
    if num.verbosity > 0
        ...
        print("\n At the right wall, not at corner \n")
    ...
    @testset "At the wall" begin
        ...
        @testset "A[j,i_corner_vecb_right]" begin
            @test A[tmp,i_corner_vecb_right] ≈ 2.0 atol = test_tolerance
        end
    ...
    print("\n A i_corner_vecb_right",A[i_corner_vecb_right,:],"\n")
