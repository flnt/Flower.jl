...
for (i,n) in enumerate(npts)
    ...
    BC = Boundaries(left = Dirichlet(val = sign_Poisson * 10.0 * sign_left), # -lapl p = f for SPD so -10
    bottom = Dirichlet(val = sign_Poisson * 10.0 * sign_bottom),
    right = Dirichlet(val = sign_Poisson * 10.0 * sign_right),
    top = Dirichlet(val = sign_Poisson * 10.0 * sign_top)) #border boundaries
    ...
    @testset "BC" begin
        ...
        @testset "Right border rhs" begin
            @test vecb_R(rhs,gp)./gp.dx[:,end] ≈ sign_Poisson * 10 * sign_right .* ones(gp.ny) atol = test_tolerance
        end

        @testset "Bottom border rhs" begin
            @test vecb_B(rhs,gp)./gp.dx[1,:] ≈ sign_Poisson * 10 * sign_bottom .* ones(gp.nx) atol = test_tolerance
        end

        @testset "Top border rhs" begin
            @test vecb_T(rhs,gp)./gp.dx[end,:] ≈ sign_Poisson * 10 * sign_bottom .* ones(gp.nx) atol = test_tolerance
        end
    end # BC
    ...
    @testset "Corner border" begin
        ...
        @testset "A[i_corner_vecb_left,tmp]" begin
            @test A[i_corner_vecb_left,tmp] ≈ 0.0 atol = test_tolerance
        end

        @testset "A[i_corner_vecb_left,i_corner_vecb_left]" begin
            @test A[i_corner_vecb_left,i_corner_vecb_left] ≈ 1.0 * dx0 atol = test_tolerance
        end

        @testset "A[i_corner_vecb_bottom,tmp]" begin
            @test A[i_corner_vecb_bottom,tmp] ≈ 0.0 atol = test_tolerance
        end

        @testset "A[i_corner_vecb_bottom,i_corner_vecb_bottom]" begin
            @test A[i_corner_vecb_bottom,i_corner_vecb_bottom] ≈ 1.0 * dx0 atol = test_tolerance
        end
