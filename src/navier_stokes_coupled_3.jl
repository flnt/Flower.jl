...
function set_crank_nicolson_block(bc_type, num, 
    ...
    data_A[1,1] = pad_crank_nicolson(opC_u.M .- τ .* Lu_fs, grid_u, τ)
    data_A[1,2] = - τ .* bc_Lu_fs
    ...
    data_A[3,3] = pad_crank_nicolson(opC_v.M .- τ .* Lv_fs, grid_v, τ)
    data_A[3,4] = - τ .* bc_Lv_fs
...
function projection_fs!(num, grid, geo, grid_u, geo_u, grid_v, geo_v, ph,
    ...
    rhs_uvϕ.data[1] .+= -τ .* (opC_u.AxT * opC_u.Rx * pD.data[1] .+ no_b0_u * opC_u.Gx * pD.data[2])
    rhs_uvϕ.data[1] .+= τ .* σ .* b0_u * opC_u.Gx * vec(grid.κ)
    rhs_uvϕ.data[2] .+= b0_u * opC_u.Gx * (pD.data[2] .+ σ .* vec(grid.κ))
    rhs_uvϕ.data[3] .+= -τ .* (opC_v.AyT * opC_v.Ry * pD.data[1] .+ no_b0_v * opC_v.Gy * pD.data[2])
    rhs_uvϕ.data[3] .+= τ .* σ .* b0_v * opC_v.Gy * vec(grid.κ)
    rhs_uvϕ.data[4] .+= b0_v * opC_v.Gy * (pD.data[2] .+ σ .* vec(grid.κ))
