...
function solve_one_fluid_NS!(
    ...
    if num.pressure_velocity_coupling == 0
        ...
        if num.prediction == "PmIIimposedpressure_nodiv_4"
            ...
            # tmp_vec_u .= 0.0
            get_height!(grid_u.LS[1],grid_u.ind,grid_u.dx,grid_u.dy,grid_u.LS[end].geoL,tmp_vec_u) #here tmp_vec_p solid #TODO geoS ??

            init_fields_multiple_levelsets!(num,uD,u,tmp_vec_u,BC_u,grid_u,num.pres_intfc,"uL")

            tmp_vec_v = zeros(grid_v) 
            # tmp_vec_u .= 0.0
            get_height!(grid_v.LS[1],grid_v.ind,grid_v.dx,grid_v.dy,grid_v.LS[end].geoL,tmp_vec_v) #here tmp_vec_p solid #TODO geoS ??
