#From  JuliaLinearAlgebra IterativeSolvers.jl 
# export bicgstabl, bicgstabl_flower!, bicgstabl_iterator, bicgstabl_iterator_flower!, BiCGStabIterable_Flower
# using Printf
import Base: iterate
# # using IterativeSolvers
# import ConvergenceHistory: reserve!
# using IterativeSolvers
import IterativeSolvers: reserve!,nextiter!,setconv,shrink!

mutable struct BiCGStabIterable_Flower{precT, matT, solT, vecT <: AbstractVector, smallMatT <: AbstractMatrix, realT <: Real, scalarT <: Number}
    A::matT
    l::Int
...
function bicgstabl_iterator_flower!(x, A, b, l::Int = 2;
    ...
    BiCGStabIterable_Flower(A, l, x, r_shadow, rs, us,
...
@inline converged(it::BiCGStabIterable_Flower) = it.residual ≤ it.tol
@inline start(::BiCGStabIterable_Flower) = 0
@inline done(it::BiCGStabIterable_Flower, iteration::Int) = it.mv_products ≥ it.max_mv_products || converged(it)

function iterate(it::BiCGStabIterable_Flower, iteration::Int=start(it),norm_type=Inf)
    ...
    it.ω = it.γ[it.l]
    it.residual = norm(view(it.rs, :, 1),norm_type)
...
# Same as [`bicgstabl_flower!`](@ref), but allocates a solution vector `x` initialized with zeros.
# # """
# # bicgstabl(A, b, l = 2; kwargs...) = bicgstabl_flower!(zerox(A, b), A, b, l; initial_zero = true, kwargs...)


"""
    bicgstabl_flower!(x, A, b, l; kwargs...) -> x, [history]
...
"""
function bicgstabl_flower!(x, A, b, l = 2;
                    abstol::Real = zero(real(eltype(b))),
                    reltol::Real = sqrt(eps(real(eltype(b)))),
