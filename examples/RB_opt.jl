...
@. my_costfunc(γ, temp_des, ls_des, temp_curr, ls_curr) = sum(γ[1]*abs(temp_des - temp_curr) + γ[2]*abs(ls_des - ls_curr))
...
function fg2!(F, G, x, num, basis, γ, opt_p, opt_S, opt_L, opt_u, fwd_des, fwdS_des, fwdL_des)
    ...
    if F != nothing
      value = my_costfunc(γ, fwdS_des.T[end,:,:], fwd_des.u[1,end,:,:], fwdS.T[end,:,:], fwd.u[1,end,:,:])
      @show (value)
      return value
    end
  end
