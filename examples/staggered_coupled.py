...
def apply_font(fontpath1):
    ...
    fe = fm.FontEntry(fname=fontpath, name="TeX Gyre Pagella Math")
...
if subplots_mode:
    ...
    fig, axs = plt.subplots(
        1, 3, figsize=(12, 6), width_ratios=[1, 0.1, 1], layout="constrained"
    )
...
#
ax_left.scatter(
    x, [0] * len(x), color="None", marker="o", edgecolors=p_color, s=marker_size
)
ax_left.scatter(
    x, [3] * len(x), color="None", marker="o", edgecolors=p_color, s=marker_size
)

ax_left.scatter(
    [0] * len(x), x, color="None", marker="o", edgecolors=p_color, s=marker_size
)
ax_left.scatter(
    [3] * len(x), x, color="None", marker="o", edgecolors=p_color, s=marker_size
)
#


ax_left.scatter(
    xu, [0] * len(xu), color="None", marker=">", edgecolors=u_color, s=marker_size
)
ax_left.scatter(
    xu, [3] * len(xu), color="None", marker=">", edgecolors=u_color, s=marker_size
)
ax_left.scatter(
    [0] * len(xu), xu, color="None", marker="^", edgecolors=v_color, s=marker_size
)

x2 = np.array([0.5, 1.5, 2.5])
ax_left.scatter(
    [0] * len(x2), x2, color="None", marker=">", edgecolors=u_color, s=marker_size
)
ax_left.scatter(
    [3] * len(x2), x2, color="None", marker=">", edgecolors=u_color, s=marker_size
)
ax_left.scatter(
    x2, [0] * len(x2), color="None", marker="^", edgecolors=v_color, s=marker_size
)
ax_left.scatter(
    x2, [3] * len(x2), color="None", marker="^", edgecolors=v_color, s=marker_size
)
...
ax_left.scatter(
    [3] * len(xu), xu, color="None", marker="^", edgecolors=v_color, s=marker_size
)

xtext, ytext = 3, 0
linewidth_points = 1  # lw_inset
# ax = ax_right
dx, dy = 0.0, -linewidth_points / 72.0
...
pressure_on_border = True
# pressure_on_border = False

if pressure_on_border:
#     x = np.array([0.5, 1.5, 2.5])
#     y = x
#     X, Y = np.meshgrid(x, y)
#     Xtmp, Ytmp = 1.5, 1.5
    ax_right.scatter(1.5, 1.5, color=p_color, s=marker_size)
    ax_right.scatter(1.5, 3, color=p_color, s=marker_size)
    ax_right.scatter(1.5, 0, color=p_color, s=marker_size)
    
    ax_right.scatter(3,1.5, color=p_color, s=marker_size)
    ax_right.scatter(0,1.5, color=p_color, s=marker_size)

    ax_right.scatter(3, 3, color=p_color, s=marker_size)
    ax_right.scatter(0, 0, color=p_color, s=marker_size)
    ax_right.scatter(0, 3, color=p_color, s=marker_size)
    ax_right.scatter(3, 0, color=p_color, s=marker_size)


# Plot on the second subplot
if not pressure_on_border:
...
interior_handle = legend_ax.scatter(
    [], [], color=p_color, label="bulk pressure", s=marker_size
)
u_handle = legend_ax.scatter(
    [], [], color=u_color, marker=">", label="bulk u", s=marker_size
)
v_handle = legend_ax.scatter(
    [], [], color=v_color, marker="^", label="bulk v", s=marker_size
)
bc_handle_p = legend_ax.scatter(
    [], [], color="none", edgecolors=p_color, label="pressure BC", s=marker_size
)
bc_handle = legend_ax.scatter(
    [], [], color="None", marker=">", edgecolors=u_color, label="u BC", s=marker_size
)
bc_handle_v = legend_ax.scatter(
    [], [], color="None", marker="^", edgecolors=v_color, label="v BC", s=marker_size
)
...
# plt.tight_layout()
# Save the figure
pressure_on_border_str = ''
if pressure_on_border:
    pressure_on_border_str = "_pressure_on_border"
plt.savefig(
    "staggered_coupled_comparison"+pressure_on_border_str+".pdf",
    transparent=True,
)

plt.savefig(
    "staggered_coupled_comparison"+pressure_on_border_str+".svg",
    transparent=True,
)
...
# plt.savefig(
#     "staggered_coupled_comparison"+pressure_on_border_str+'_tight'+".pdf",
#     transparent=True,
# )

# plt.savefig(
#     "staggered_coupled_comparison"+pressure_on_border_str+'_tight'+".svg",
#     transparent=True,
# )
