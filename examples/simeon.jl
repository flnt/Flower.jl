...
function IC(r)
    phi = sqrt.(A^2 .- r.^2) # A/2/tan(alpha).-1/tan(alpha)*r # sqrt.(A^2 .- r.^2)
    kappa = A^2 ./ sqrt.(A^2 .- r.^2).^3 # (1/tan(alpha)+1/tan(alpha)^3)./r # A^2 ./ sqrt.(A^2 .- r.^2).^3
    return phi,kappa
end
...
function genf_interface(α, κ, x, x0, dim)
    β = α + pi/2
    if dim<0.5 # 2D
        if (x-x0)*β < -0.01
            V = (α<0)*cos(β)/(1-cos(β)*cbrt((x-x0)/sin(β))) # simple dissolution model normal velocity
        else
            V = (α<0)/(1-cbrt(cos(β)^2/κ)) # simple dissolution model normal velocity near a pole
        end
    else # axisymmetric
        if (x-x0)*β < -0.01
            V = (α<0)*cos(β)/(1-cos(β)*cbrt((x-x0^2/x)/sin(β))) # simple dissolution model normal velocity
        else
            if x0>0
                V = (α<0)/(1-cbrt(2*cos(β)^2/κ)) # simple dissolution model normal velocity near a pole
            else
                V = (α<0)/(1-cbrt(cos(β)^2/κ)) # simple dissolution model normal velocity near a pole
            end
        end
...
if cent<0.5
    ...
    ax = Axis(f1[1,1], aspect=DataAspect(), xlabel=L"x-x_0", ylabel=L"y-y_0", xtickalign=0,  ytickalign=0)
end
...
# ylims!(-L,A)
if cent<0.5
    lines!(ax,peakx[1:20:end],peaky[1:20:end], color=:green, linestyle=:dash, linewidth = 3)
end
f1
